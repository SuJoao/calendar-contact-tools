...
export function vcfOptionsMarkup(path: string): string {
  ...
  if (path === '/vcf-duplicate-remover')
    return `<div class="notice"><strong>No contact is removed automatically.</strong> Indexed UID, email, phone, name, address, organization, and birthday signals produce explainable candidates. Review and resolve each group before downloading.</div>`;
  if (path === '/vcf-merge')
